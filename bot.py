...
CHANNEL_ID = 1490680218178813952
...
async def slash_id(
    ...
    try:
        ...
        if clan not in clans_existants:
            await interaction.response.send_message("❌ Clan introuvable dans le sheet.", ephemeral=True)
            return

        produits_disponibles = get_products_for_clan(clan)
        if produit_val not in produits_disponibles:
            await interaction.response.send_message(
                f"❌ Le produit **{produit_val}** n'est pas disponible pour **{clan}**.",
                ephemeral=True
            )
            return

        sheet_name = find_sheet_for_clan_and_product(clan, produit_val)
        if not sheet_name:
            await interaction.response.send_message("❌ Impossible de trouver l'onglet correspondant.", ephemeral=True)
            return

        if action_val == "set":
            ok, resultat = remplacer_game_id(sheet_name, clan, produit_val, identifiant)
        elif action_val == "add":
            ok, resultat = ajouter_game_id(sheet_name, clan, produit_val, identifiant)
        elif action_val == "sup":
            ok, resultat = supprimer_game_id(sheet_name, clan, produit_val, identifiant)
        else:
        ...
        print("Erreur /id :", e)
        await interaction.response.send_message("❌ Erreur lors de la modification du Game ID.", ephemeral=True)


@slash_id.autocomplete("clan")
async def clan_autocomplete(interaction: discord.Interaction, current: str):
    try:
        clans = get_all_clan_names()
        current_lower = current.lower()

        results = [
            app_commands.Choice(name=clan, value=clan)
            for clan in clans
            if current_lower in clan.lower()
        ]

        return results[:25]
    except Exception as e:
        print("Erreur autocomplete clan :", e)
        return []
