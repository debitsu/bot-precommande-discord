...
async def on_message(message):
    ...
    if contenu.startswith("!id "):
        try:
            args = shlex.split(contenu)

            if len(args) < 6:
                await message.reply(
                    '❌ Format : !id <scope> <set|add|del> <gorgon|imp|imp+gorgon> "NomClan" valeur\n'
                    'Exemples :\n'
                    '!id 1clan set gorgon "Alpha" id1,id2,id3\n'
                    '!id 2clans add imp "Beta" id4\n'
                    '!id 3clans del imp+gorgon "Gamma" id2',
                    mention_author=False
                )
                return

            _, scope_raw, action, produit_raw, clan_name, valeur = args[0], args[1].lower(), args[2].lower(), args[3].lower(), args[4], args[5]

            if scope_raw not in SCOPE_MAP:
                await message.reply("❌ Scope invalide. Utilise : 1clan, 2clans, 3clans.", mention_author=False)
                return

            sheet_name = SCOPE_MAP[scope_raw]
            ...
            if produit_raw not in PRODUCT_MAP:
                await message.reply("❌ Produit invalide. Utilise : gorgon, imp, imp+gorgon.", mention_author=False)
                return

            if action == "set":
                ok, resultat = remplacer_game_id(sheet_name, clan_name, produit_raw, valeur)
                if ok:
                    await message.reply(
                        f"✅ Game ID remplacé pour **{clan_name}** / **{produit_raw}** dans **{sheet_name}**.\n"
                        f"Nouveau contenu : `{resultat}`",
                        mention_author=False
                    )
                else:
            ...
            if action == "add":
                ok, resultat = ajouter_game_id(sheet_name, clan_name, produit_raw, valeur)
                if ok:
                    await message.reply(
                        f"✅ Game ID ajouté pour **{clan_name}** / **{produit_raw}** dans **{sheet_name}**.\n"
                        f"Nouveau contenu : `{resultat}`",
                        mention_author=False
                    )
                else:
            ...
            if action == "del":
                ok, resultat = supprimer_game_id(sheet_name, clan_name, produit_raw, valeur)
                if ok:
                    await message.reply(
                        f"✅ Game ID supprimé pour **{clan_name}** / **{produit_raw}** dans **{sheet_name}**.\n"
                        f"Nouveau contenu : `{resultat}`",
                        mention_author=False
                    )
                else:
            ...
            await message.reply("❌ Action invalide. Utilise : set, add, del.", mention_author=False)

        except Exception as e:
            print("Erreur !id :", e)
            await message.reply("❌ Erreur lors de la modification de Game ID.", mention_author=False)
        return

    if contenu.lower() == "!syncclans":
        try:
