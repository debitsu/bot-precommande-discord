...
SPREADSHEET_ID = "COLLE_ICI_L_ID_DU_TABLEUR"
...
def creer_embed_stats(sheet_name, categories, total_prix):
    total_gorgon = len(categories["gorgon"])
    total_imp = len(categories["imp"])
    total_combo = len(categories["combo"])

    objectif = OBJECTIF_TOTAL
    pourcentage = (total_prix / objectif) * 100 if objectif > 0 else 0

    total_prix_affiche = int(total_prix) if total_prix.is_integer() else round(total_prix, 2)
    pourcentage_affiche = round(pourcentage, 2)

    embed = discord.Embed(
        title="📊 Statistiques",
        description=f"Résumé actuel de l'onglet **{sheet_name}**",
        color=0x3498db
    )

    embed.add_field(name="🟢 Gorgon", value=str(total_gorgon), inline=False)
    embed.add_field(name="🔴 Imp", value=str(total_imp), inline=False)
    embed.add_field(name="🟡 Combo", value=str(total_combo), inline=False)
    embed.add_field(
        name="💰 Progression financière",
        value=f"{total_prix_affiche} / {objectif} ({pourcentage_affiche}%)",
        inline=False
    )
...
class CategoryButtonsView(discord.ui.View):
    ...
    async def bouton_stats(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            categories, total_prix = recuperer_donnees(self.sheet_name)
            embed = creer_embed_stats(self.sheet_name, categories, total_prix)
            await interaction.response.send_message(embed=embed, ephemeral=True)
        except Exception as e:
            print("Erreur bouton stats :", e)
            await interaction.response.send_message("❌ Erreur lors de la récupération des statistiques.", ephemeral=True)
...
class SheetSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=f"Tu as choisi **{sheet_name}**.\nClique maintenant sur une catégorie ou sur les stats.",
            color=0x5865F2
        )
...
async def on_message(message):
    ...
    if message.content.strip().lower() == "!menu":
        embed = discord.Embed(
            ...
            description="Choisis d’abord un onglet, puis une catégorie ou les stats.",
            color=0x5865F2
        )
        embed.add_field(
