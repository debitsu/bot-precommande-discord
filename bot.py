...
class IDModal(discord.ui.Modal, title="Modifier Game ID"):
    ...
    ids = discord.ui.TextInput(
        label="Game ID(s)",
        placeholder="Ex: id1,id2,id3 ou id4",
        required=True,
        style=discord.TextStyle.paragraph,
        max_length=1000
    )

    def __init__(self, scope, action, produit):
        super().__init__()
        self.scope = scope
        self.action = action
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            sheet_name = SCOPE_MAP[self.scope]
            clan_name = self.clan.value.strip()
            valeur = self.ids.value.strip()

            if self.action == "set":
                ok, result = remplacer_game_id(sheet_name, clan_name, self.produit, valeur)
            elif self.action == "add":
                ok, result = ajouter_game_id(sheet_name, clan_name, self.produit, valeur)
            elif self.action == "del":
                ok, result = supprimer_game_id(sheet_name, clan_name, self.produit, valeur)
            else:
            ...
            print("Erreur modal !id :", e)
            await interaction.response.send_message("❌ Erreur lors de la modification de Game ID.", ephemeral=True)
...
class ProduitSelect(discord.ui.Select):
    def __init__(self, scope, action):
        ...
        options = [
            discord.SelectOption(label="Gorgon", value="gorgon"),
            discord.SelectOption(label="Imp", value="imp"),
            discord.SelectOption(label="Gorgon + Imp", value="imp+gorgon"),
        ]

        super().__init__(
            placeholder="Choisis un produit",
            min_values=1,
            max_values=1,
            options=options
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_modal(IDModal(self.scope, self.action, produit))
...
class ActionSelect(discord.ui.Select):
    def __init__(self, scope):
        ...
        options = [
            discord.SelectOption(label="Set (remplacer)", value="set"),
            discord.SelectOption(label="Add (ajouter)", value="add"),
            discord.SelectOption(label="Del (supprimer)", value="del"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view.add_item(ProduitSelect(self.scope, self.values[0]))
        await interaction.response.send_message("Choisis le produit :", view=view, ephemeral=True)


class ScopeSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="1 clan", value="1clan"),
            discord.SelectOption(label="2 clans", value="2clans"),
            discord.SelectOption(label="3 clans", value="3clans"),
        ]

        super().__init__(
            placeholder="Choisis un scope",
            min_values=1,
            max_values=1,
            options=options
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        view = discord.ui.View(timeout=120)
        view.add_item(ActionSelect(self.values[0]))
        await interaction.response.send_message("Choisis une action :", view=view, ephemeral=True)


class IDMenuView(discord.ui.View):
    def __init__(self):
...
async def on_message(message):
    ...
    if contenu == "!id":
        embed = discord.Embed(
            title="⚙️ Gestion des Game ID",
            description="Choisis le scope, l’action, puis le produit.",
            color=0x5865F2
        )
        embed.add_field(
            name="Scopes disponibles",
            value="1 clan\n2 clans\n3 clans",
            inline=False
        )
        await message.channel.send(embed=embed, view=IDMenuView())
        return

    if contenu == "!syncclans":
        try:
            ...
            await message.reply(message_retour, mention_author=False)

        except Exception as e:
            print("Erreur syncclans :", e)
            await message.reply("❌ Erreur lors de la synchronisation de la feuille CLANS.", mention_author=False)
        return
